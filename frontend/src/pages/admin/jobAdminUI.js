...
import { setJobModal } from "../../redux/jobModalSlice";
...
function JobAdmin() {
  ...
  return loading === true ? (
    ...
    <div className="flex flex-col ">
      ...
      <div className="flex flex-wrap">{data.map((item) => <AppliedJobs name={item.name} degree={item.degree}  status={item.approved} no={item.JobID} email={item.jobEmail}/>)}</div>
      </div>
  );
}
